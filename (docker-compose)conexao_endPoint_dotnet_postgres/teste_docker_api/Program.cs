...
app.MapPost("/user", async()=>{
    try
    {
        ...
        using(var conn = new NpgsqlConnection("Host=postgres_server;Port=5432;Database=user_testes;Username=postgres;Password=1234")){
            conn.Open();
    ...
    {
        throw new Exception("Internal errors, try again", e);
    }
